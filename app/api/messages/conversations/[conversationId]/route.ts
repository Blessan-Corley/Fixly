...
function isAccessDeniedError(error: unknown): boolean {
  return error instanceof Error && /access denied|unauthorized/i.test(error.message);
}
...
export async function GET(request: NextRequest, props: RouteContext) {
  ...
  try {
    ...
    if (!conversationId) {
      return apiValidationError('Conversation ID is required');
    }
    ...
    const before = toTrimmedString(searchParams.get('before'));
    ...
    const filteredMessages = before
      ? allMessages.filter((message) => {
          ...
          return Number.isFinite(timestamp.getTime()) && timestamp.getTime() < Number(before);
        })
      : allMessages;
    ...
    const items = filteredMessages.slice(startIndex, endIndex).map(normalizeMessage);
    ...
    return apiSuccess({
      ...
      conversation: {
        ...
        conversationType: toTrimmedString(conversation.conversationType) || 'direct',
      },
    });
  } catch (error: unknown) {
    logger.error({ error }, 'Get conversation thread route error');
