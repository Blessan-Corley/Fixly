...
import { normalizeUsername, parseCachedPayload, type ProfileUser, type UsernameParams } from './profile.helpers';
...
export async function GET(request: Request, props: UsernameParams) {
  ...
  try {
    ...
    try {
      ...
      if (parsedCache) {
        return respond(parsedCache, 200, { headers: { 'X-Cache': 'HIT', 'Cache-Control': `max-age=${cacheTTL}` } });
      }
      if (cachedProfile) await redisUtils.del(cacheKey);
    ...
    const [jobStats, recentReviews, reviewStats] = await Promise.all([
      ...
      Review.find({ reviewee: user._id, status: 'published', isPublic: true })
    ...
    const profileData: Record<string, unknown> = {
      ...user,
      stats: { ...(jobStats?.[0] || {}), reviews: reviewStats },
      recentReviews,
    };
    ...
    const responsePayload = { success: true, user: profileData };
