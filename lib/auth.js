...
import { getAuthContext, clearAuthContext } from './authContext';
...
export const authOptions = {
      ...
      GoogleProvider({
        ...
        authorization: {
          params: {
            ...
            // Custom state will be added by the signin function
          }
        }
      })
    ] : []),
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          return {
            ...
            picture: user.profilePhoto?.url || user.picture,
  ...
  callbacks: {
    async signIn({ user, account, profile, cookies }) {
      ...
      // ✅ COMPREHENSIVE GOOGLE SIGNIN/SIGNUP LOGIC
      if (account?.provider === 'google') {
        try {
          // Get the stored auth context from cookie (signup vs signin)
          const authContext = cookies?.fixly_auth_context || null;
          // console.log('🔄 Processing Google auth for:', user.email, '| Context:', authContext || 'unknown');

          // First, try to connect to database
          await connectDB();
          // console.log('✅ Database connected for Google auth');
          ...
          // Note: Cookie will auto-expire after 5 minutes, no need to manually clear

          if (existingUser) {
            // console.log('👤 Existing user found:', existingUser.email, '| Role:', existingUser.role);

            // ✅ SECURITY: Check if user is banned or inactive
            ...
            if (!existingUser.isActive || existingUser.deletedAt) {
              console.log('❌ Inactive user attempted Google login');
              return '/auth/error?error=AccountInactive';
            }

            // ✅ EDGE CASE 1: Existing user trying to SIGN UP again
            // Redirect them to dashboard instead of showing error
            if (authContext === 'signup') {
              console.log('✅ Existing user tried to signup, allowing signin and will redirect to dashboard');
            }

            // ✅ EDGE CASE 2: Existing user SIGNING IN (normal flow)
            // This is the expected flow for returning users
            ...
            // If Google ID is missing, add it and mark as Google user
            if (!existingUser.googleId) {
              updateData.googleId = account.providerAccountId;
              updateData.emailVerified = true;
              updateData.isVerified = true;
              updateData.authMethod = 'google';
              updateData.$addToSet = { providers: 'google' };
            }

            await User.findByIdAndUpdate(existingUser._id, updateData);

            // Set user properties from database for JWT
            ...
            // Check if registration is complete (has role and non-temporary username)
            user.isRegistered = !!(existingUser.role && existingUser.username && !existingUser.username.startsWith('tmp_'));
            user.needsOnboarding = !user.isRegistered;

            console.log('✅ Existing user auth successful, isRegistered:', user.isRegistered, '| needsOnboarding:', user.needsOnboarding);
            return true;

          } else {
            // NO EXISTING USER FOUND

            // ✅ EDGE CASE 3: New user trying to SIGN IN
            // Show error and redirect to signup page
            if (authContext === 'signin') {
              console.log('❌ New user tried to signin without account');
              return `/auth/error?error=AccountNotFound&email=${encodeURIComponent(profile.email)}&name=${encodeURIComponent(profile.name || '')}`;
            }

            // ✅ EDGE CASE 4: New user SIGNING UP (normal flow)
            // Create account with temporary username, user will complete profile
            console.log('🆕 Creating new Google user for signup:', user.email);
            ...
            try {
              ...
              const newUser = await User.create({
                ...
                isVerified: true,
                emailVerified: true,
                isRegistered: false, // Key: Mark as incomplete - needs to complete profile
              ...
              console.log('✅ New Google user created successfully, needs to complete profile');
              return true;
              ...
              return '/auth/error?error=SignupFailed';
            }
          }
        } catch (error) {
          ...
          // Return error URL to redirect to error page with user-friendly message
          // Don't expose internal errors like "DatabaseConnectionFailed"
    ...
    async redirect({ url, baseUrl }) {
      ...
      // Default safe redirect to dashboard for successful auth
