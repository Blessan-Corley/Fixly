  ...
  callbacks: {
    async signIn({ user, account, profile, request }) {
      ...
      if (account?.provider === 'google') {
        try {
          ...
          if (existingUser) {
            ...
            try {
              ...
              return true;
                ...
                // Even if DB creation fails, we can allow signin to prevent blocking the user,
                // but they won't be able to complete signup. Better to fail gracefully.
                // However, based on your current setup, returning true and handling the lack
                // of DB user in the signup route might be acceptable.
                // Let's proceed but log the critical issue.
                console.warn('⚠️ Proceeding with Google signin without DB user. This will likely cause issues in /api/auth/signup.');
                // Set fallback IDs
                user.id = account.providerAccountId; // Use Google ID as fallback
                user.isRegistered = false;
          ...
          // For new users, don't block signin on database errors if possible, but log it critically
          console.log('⚠️ Database error during Google signin. Proceeding with limited user data.');
          // Set fallback IDs and flags
          ...
          user.needsOnboarding = true;
          user.isRegistered = false;
          // Critical: Set a fallback ID. Using Google ID is not ideal for session.user.id
          // as your API expects MongoDB ObjectId. This might still cause issues.
          // The creation above should prevent this path ideally.
          user.id = account.providerAccountId; 

          console.log('✅ Fallback Google user setup complete (but DB user creation likely failed)');
          return true; // Allow signin, but expect issues in signup completion
        }
      }
    ...
    async jwt({ token, user, account, trigger }) {
      ...
      if (trigger === 'update' && token.id && token.id.length === 24) { // Valid MongoDB ObjectId
        try {
          ...
          // Check if cache is valid with session version
    ...
    async session({ session, token }) {
      ...
      if (session.user) {
        ...
        // Google users who haven't completed signup will now have a valid ObjectId
