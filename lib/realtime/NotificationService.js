...
class NotificationService {
  ...
  async storeNotification(userId, notification) {
    try {
      ...
      console.log(`📤 Stored notification for user ${userId}:`, notification.title);
      return true;
    } catch (error) {
  ...
  async updateNotificationStatus(userId, notificationId, updates) {
    try {
      ...
      await User.findByIdAndUpdate(userId, {
        $set: {
          'notifications.$[elem].read': updates.read || false,
          'notifications.$[elem].readAt': updates.readAt || new Date()
        }
      }, {
        arrayFilters: [{ 'elem._id': notificationId }]
      });
      
      console.log(`📝 Updated notification ${notificationId} for user ${userId}:`, updates);
      return true;
    } catch (error) {
  ...
  async updateAllNotificationStatus(userId, updates) {
    try {
      ...
      await User.findByIdAndUpdate(userId, {
        $set: {
          'notifications.$[].read': updates.read || false,
          'notifications.$[].readAt': updates.readAt || new Date()
        }
      });
      
      console.log(`📝 Updated all notifications for user ${userId}:`, updates);
      return true;
    } catch (error) {
